...
def task(
    ...
    timeout_seconds: float = 30.0,
    retry_count: int = 0,
    input_validator: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None,
):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # Register the function with the task runner
        ...
        # Add metadata to the function for skill integration
        func._task_metadata = {
            "name": task_name,
            "description": description or func.__doc__ or "",
            "is_async": is_async,
...
def register_task_from_module(module) -> None:
    """Register all task functions from a module."""
    for attr_name in dir(module):
        attr = getattr(module, attr_name)
        if hasattr(attr, '_task_metadata'):
            metadata = attr._task_metadata
            # Skip if already registered
            if metadata['name'] not in default_runner._tasks:
                default_runner.register(
                    name=metadata['name'],
                    func=attr,
