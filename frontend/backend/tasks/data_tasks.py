...
# Backend is now at frontend/backend/, so docs is at frontend/docs (one level up)
CONTENT_DIR = Path(__file__).parent.parent.parent / "docs"
# External content is at root level (two levels up from frontend/)
EXTERNAL_CONTENT_DIR = Path(__file__).parent.parent.parent.parent / "external-content"


@task(
    name="fetch_chapter_content",
    description="Fetch content from a specific chapter of the textbook with optional section filtering",
...
@task(
    ...
    description="Search the textbook content for relevant passages based on query",
...
@task(
    ...
    description="Look up a term in the glossary with suggestions for similar terms",
...
@task(
    name="list_chapters",
    description="List all available chapters in the textbook with ID and title",
    timeout_seconds=10.0
)
